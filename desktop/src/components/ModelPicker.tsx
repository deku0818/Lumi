    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="group flex items-center gap-1 max-w-52 px-2 py-1 rounded-lg text-xs text-muted-foreground hover:text-ink hover:bg-canvas/60 transition outline-none"
        >
      ...
      <DropdownMenuContent side="top" align="start" className="w-64 max-h-80 overflow-auto">
        {providers.length === 0 && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground">{t('providers.none')}</div>
        )}
        {providers.map((p, i) => (
          <DropdownMenuGroup key={p.id}>
            ...
            {p.models.length === 0 && (
              <div className="px-2 pb-1 text-xs text-muted-foreground/60">{t('model.noModels')}</div>
            )}
            {p.models.map((m) => {
