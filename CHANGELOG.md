...
### Fixed
- **运行计时不再切页归零**（`App.tsx` / `gateway/session.py`）— 「正在处理…」的计时原是组件内自增计数，切到别的会话再回来组件重挂载就从 0 重数。现在本轮开始时刻存进会话状态、计时由「现在 − 开始时刻」推算；后端 `_RunState.begin` 记下开轮时刻并随 `gateway.ready.run_started_at` 下发，重连 / 重载后同样续算。观测运行中的定时任务改取其 `started_at`。

### Changed
- **时长满 60 进位**（`lib/utils.ts` `fmtDuration`）— 运行计时、后台任务耗时、定时任务执行记录耗时统一显示为 `42s` / `1m 5s` / `1h 2m 3s`，秒恒显示，长任务不再像卡住。

## [0.2.128] - 2026-09-18

### Added
- **工具调用参数可见**（`App.tsx` `ToolRow` + 新增 `shell.ts`）— 收起态每行第二行直接显示关键参数：bash 命令（轻量 shell 着色，复用代码块 `--hl-*` 色板）、read/write/edit 路径、grep/glob 搜索词与目录；非默认选项以 chip 呈现（超时、后台、行范围、glob、忽略大小写、全部替换、+/- 行数）。MCP 等未登记工具回退为全量键值，不再只取第一个字符串字段。
- **展开态整行合成一张卡**— 标题行作卡头（悬停只亮卡头），发丝线下是命令 / 路径（或键值表）、虚线下接输出或 diff，chip 挪到标题行，复制按钮悬停才出现；多行命令续行悬挂对齐首行命令。开合走 grid-rows 高度过渡，展开体首次打开才挂载，成组展开不再为每行预渲染输出 / diff。

### Changed
- 侧栏项目树行首圆点改为「执行中实心金点缓慢明灭」，当前会话靠行底色区分。
- 确认弹窗去掉右上角关闭钮与底栏灰底分隔，取消按钮改为浅填充。
- `argText` 下沉到 `lib/utils`，审批弹窗与工具行键值共用。

## [0.2.127] - 2026-09-17

### Fixed
- **离开久了回来不再显示「没有项目」**（`App.tsx` / `gateway.ts`）— 开局查默认项目抢在控制连接握手前发出必然落空：设了默认项目也被送去项目选择器，且列表为空、无人重试。现在等本地连接连上再查；项目列表依赖连接态，连上 / 重连成功即重拉，未拉到时列表区留空而非谎报「还没有项目」。
- **连接停摆后自动恢复**（`Gateway.wake()`）— 退避重连约 15s 耗尽后连接停在 failed，此前只能手动点重连；现在窗口重获焦点 / 网络恢复时自动唤醒（主动关闭与令牌无效除外），睡眠、断网回来即自愈。
- 项目主页四卡、定时任务执行记录、飞书绑定项目选择改为挂在机器连接态上取数，断线期间进入不再定格成空态。

### Changed
- 聊天页标题栏显示当前会话标题、所在机器与项目。
- 后台任务「清空已完成」改为垃圾桶图标按钮；右栏卡片投影不再被滚动容器切出硬边。
- 侧栏会话圆点与标题拉开间距，飞书会话图标与普通会话圆点同中心线对齐。

## [0.2.126] - 2026-09-16

### Changed
- **侧栏底栏显示当前项目与连接**（`Sidebar.tsx` `AccountMenu`）— 原模型名改为「机器图标（带连接态点）+ 项目名 + 靠右连接名」，跟随当前会话；点项目组头「＋」新建后一眼可见落在哪个项目、哪台机器。
- 侧栏层级对齐：项目组头与「新对话 / 项目 / 定时任务」左移至同一起点，会话条目相对缩进一级；会话标题字号 14px → 13px，与正文一致。
...
### Changed
- **后台任务栏按状态分组**（`BgTasksDrawer.tsx` 重写）— 此前运行中与终态任务在同一条流水里平铺，而终态每会话可攒到 20 条（`_TERMINAL_CAP`），几轮 `simplify` 下来正在跑的那几个就被顶出视野了。现在运行中是一摞紧凑卡片、终态整体收进「已完成 N」一行折叠开关 +「清除」，展开后是一行一个的紧凑列表。卡片同时从三段式压到三行：名称 + 光点 + **常驻停止钮** / `kind · 时长 · 活动 · N 次工具` / 进度条或最新一行动静——停止不再藏在展开之后（正在跑的任务最常做的操作就是叫停它），任务内容、退出码、错误与完整输出改为点开卡片 / 已完成行就地展开，同时只开一个（栏只有 256 宽，两份摊开就得来回滚）。
- **输出轮询多加一道门** — 原先只看右栏是否展开，现在还要「详情摊开了」或「边跑边写且卡面那行要跟着走」才拉；已完成的任务不点开就不再拉它的输出文件。

### Fixed
- **`RailSection` 的 `headerExtra` 删除**（`RightRail.tsx`）— 唯一的使用者是后台任务节头的「清除已完成」，该按钮已随分组移进「已完成」那一行，这个口子只剩死代码。
...
### Changed
- **模型改为会话属性，desktop 与飞书共用一条解析链**（新增 `sessions/session_model.py`：`会话模型 > 新会话默认` 两层 + `pin()` 固化）— 此前 desktop 的「会话模型」其实写的是全局 active，切一次影响新会话与所有跟随全局的渠道会话；而 `providers.active` 同时兼任「当前用什么」和「新会话用什么」两个角色，串味都由此而来。现在：在某个会话里选的模型就是这个会话此后一直用的模型，切走再切回来还是它；会话**第一轮开跑时**把当时生效的模型固化进 `session_meta`，此后改「新会话默认」不再波及任何聊过的会话——prompt 缓存按模型分桶，在用户背后换模型等于让整段历史被新模型重读一遍。没开跑过的空会话不固化，仍跟随默认。设置页那一行相应正名为「新会话默认模型」。
- **渠道级模型配置整块删除**（`ChannelRuntimeConfig` 只剩 `tool_mode` / `workspace`，渠道页去掉「模型来源」区块）— 渠道级固定模型会把同一渠道下每个群绑死在一档，且与会话级设定构成两套优先级。飞书换模型改为在那个群里 `/model`，每个群各记各的。老配置里残留的 `model/provider/effort` 字段被忽略，不影响加载。顺带：`BridgePool` 的身份元组退化成只剩 `workspace`，改渠道配置不再 `close_all` 重建会话池（此前会把所有飞书会话的常驻 bridge 连同在途轮一起收掉）。
- **轮首对齐下沉到 bridge 自己的入口**（`_stream_turn` / `compact_thread` 调 `align_session_model`，`_stream_user_turn` 调 `pin`）— 「跑图前先把模型对齐到本会话」是跑图的不变量，不是某个前端的：原先由 desktop / 飞书 outbound / daily_dream 各自记得调，desktop 的后台通知合成轮因此从未对齐过，跑的是 bridge 上次的模型。固化同理跟着「真人轮 vs 合成轮」这条既有分界走，不再传手工维护的 `pin` 参数——否则飞书的合成通知轮会把后台任务当用户轮、替用户钉死会话模型。

### Added
- **飞书 `/effort` 会话级思考档位**（`RUNTIME_COMMANDS` + `session_model.set_effort`）— 与 `/model` 对称、同落一条会话 meta：裸敲看本会话生效档位与该模型的可选档位（按当前模型现算，不列端点不认的值），带值只改本会话，`auto` 回到跟随模型默认。换模型一并重置档位（旧档位对新模型多半不适用）。
- **desktop 切模型前的缓存失效确认**——已开跑过的会话换模型会让完整历史被重读，弹框说明代价后再切；未开跑的会话没有缓存可废，直接切、不打扰。判据取后端下发的 `pinned` 而非「消息列表是否为空」（长会话历史加载完成前后者也是空的，那一瞬会静默切掉）。

### Fixed
- **飞书 `/effort` 裸敲把「跟随模型档位」一律报成 `auto`**——未设会话级档位时实际生效的是该模型在 profile 里的档位（desktop 选择器写的），报 `auto` 会让「明明设过 high」看起来没生效。
- **项目主页的模型选择器改的是上一个会话**——项目页还没有会话（`active` 仍指向上一个），那里选模型的意思是「我要开的新会话用它」，故改「新会话默认」且不弹确认框；同页的文件夹与审批模式早已按此取舍加了守卫，模型选择器补齐。
...
### Fixed
- **`/direct --dir /y-codespaces/ep-liftOS` 报「不认识的选项 `--codespaces/ep`」**（`relay._FLAG_DASH`）— 旗标前缀正则为容错少打一横的 `-dir` 同时接受单短横，但没限定位置，切段时把路径中间的连字符也当成了旗标分隔点。加 `(?<!\S)` 锚定：只有行首 / 空白后的短横才算前缀，路径里的 `-` 不再参与切分；破折号 / 全角变体容错不受影响。

### Fixed
- **飞书 bot 发送者显示 `用户_xxxxxx` 而非真名**（`feishu/directory.py` 的 `resolve_bot_sender`）— 群成员接口官方明确不返回 bot 成员、通讯录接口也只覆盖真人，bot 发送者两个数据源都查不到，必然落真人兜底名——机器人之间对话时模型把对方当成陌生用户。修复走第三条反查路：拿该 bot 刚发的消息 `im.message.get` 取 sender.app_id，再 `application.get` 取应用名（三跳形状均经真实 API 实测锁定），结果与真人共享同一 `open_id → 名` 缓存（同一 bot 只查一次）。查他方应用需 `admin:app.info:readonly`，已入 `OPTIONAL_SCOPES`——接入体检对未开通用户显示 warn +「其他机器人发送者的名称」降级说明，一键开通链接自动预填；未授权时兜底名改为诚实的 `机器人_xxxxxx`，不再冒充用户，授权发布后下条消息即恢复真名。体检「仅缺可选权限」分支补上「开通后需发布版本才生效」提示（缺必需权限分支本就有，不提醒的话用户开完回来看还是 warn 以为没生效）。
...
### Added
- **飞书会话来源注入 `<env>`**（`feishu/inbound.py` 的 `channel_env`）— 模型不再靠猜自己在哪个群：一行「会话来源: 飞书」+ 一级缩进子项（场景 / 群名或对方 / `chat_id`，私聊另加对方 `open_id`）缀在 os / cwd 之后、同块之内，可直接拿 id 调飞书工具发消息、拉群成员。发言人**不进**这块（群里每条都在换人，写进来等于每轮 digest 变、每轮重发整块，而"谁在说话"已由 `<sender>` 标签逐条带着）；群名解析不到时那一行整条省掉——兜底名「群_a1b2c3」会被模型当真名复述给用户。层级由渠道自己排：`context.env_extra` 收的是渲染好的条目行（含缩进）而非结构，core 不认识群聊私聊。desktop 会话恒空、块内容与改动前逐字节相同；子 agent 由 agent 工具传播（前台 / 后台两条路都带）。

### Changed
- **环境信息块改用 `<env>` 标签**（`constants.ENV_TAG`）— 原 `<system-reminder>` + 标题句「用户当前系统环境信息」全部去掉，块内只剩自解释的条目行；变更时原样重发整块，不再加「环境信息已变更，以下为最新:」提示句（后出现的那块即最新，一条代码路径）。环境是长期有效的事实、不是一次性提醒，故只有 env 换标签，skill / agent / 记忆等块仍走 `<system-reminder>`。
- 飞书会话显示名每条消息只解析一次（`_resolve_title`），sidecar 与 `<env>` 共用同一结果：两处各解析会绕开无名群的 5 分钟退避冷却、刷成每消息一次 API。同时把判定统一为 `!= "p2p"` 即群——话题群此前走私聊分支，会把发言人名当群名写进会话列表。
