"""模型供应商 profile CRUD：``provider_store`` 之上的 wire 形状层。

全部是无状态的模块级函数（读写的是持久化的 profile 表，不是某个 bridge 的运行态），
desktop RPC（``providers_rpc``）直接调用；bridge 只在建桥时读一次「新会话默认」。
"""
...
def search_catalog(query: str) -> dict:
    ...
    levels 与 list_providers 的 thinking.levels 同源（``levels_for``），即**可选
...
def set_effort(provider_id: str, model: str, level: str) -> dict:
...
def set_classifier(provider_id: str, model: str) -> dict:
    ...
    """
    return {"classifier": provider_store.set_pointer("classifier", provider_id, model)}


def set_titler(provider_id: str, model: str) -> dict:
...
def set_provider(provider_id: str, model: str) -> dict:
    """把「新会话默认模型」切到 (provider, model)，持久化。

    **不动任何已有会话**——包括调用连接当前这个。模型是会话属性，已开跑的会话
...
def save_provider(profile: dict) -> dict:
