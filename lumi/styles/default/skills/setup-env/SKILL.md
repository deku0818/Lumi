---
name: setup-env
description: 检查并安装 Lumi 干活所需的本地环境工具（uv、Node.js、ripgrep），以及飞书机器人的全程接入（凭证、配置、权限体检、妙记）。用户说「初始化 Lumi」「配置环境」「装依赖」「第一次用要准备什么」「接飞书」「配置飞书机器人」「开妙记纪要」，或任务因 uv/node/npm/python 命令找不到而失败、装不上或找不到 lark-cli、飞书机器人不回消息时使用。
---

# Lumi 环境安装

把 agent 干活要用的工具装齐。面向零基础用户——全程你来跑命令，能代劳的都代劳。
...
Windows（shell 是 cmd，没有 `${VAR:-默认}` 语法）：
...
环境齐了之后，告诉用户下一步可以配模型或接飞书机器人。

## 接飞书机器人 / 妙记

完整接入流程（凭证 → 配置 → 权限体检 → 启用 → 妙记）单独成篇：**先读本技能目录下
的 `references/feishu.md` 再动手**，别凭记忆走——体检循环里哪些你自己跑、哪些只能
请用户点网页，那里都定好了。

## 出问题时

出了问题也是你来修——用这个技能的用户多半不懂技术，能代劳的就别把人支到别处去点。
