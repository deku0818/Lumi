...
默认情况下，Lumi 会对大多数有副作用的操作（bash 命令、文件写入）弹出审批。你可以通过配置文件预先 allow 常用操作来减少审批频率。
...
当工具调用触发审批时，桌面端审批卡**逐个**展示本轮调用（工具名 + 逐项参数 + 风险 / 越界提示），每个调用单独选：
...
多个调用时决定完进入总览页，可点回任一项修改后提交。部分拒绝时已允许的照常执行、被拒的附说明回给模型继续；全部拒绝则结束本轮。刻意不提供「全部允许」批量捷径。

要让某类操作以后免审批，在权限配置里预先写 allow 规则（见上文）。
...
运行中也可以在输入框底栏切换审批模式，对当前运行轮后续的工具调用即时生效。
