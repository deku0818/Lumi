...
当工具需要审批时，输入框上方出现审批卡，**一次看一个调用**：
- 工具名称和逐项参数（长参数折叠，可展开全部）
- 风险提示 / 超出工作区提示
- 选项：允许、拒绝——决定后自动翻到下一个

一轮有多个调用时，全部决定完进入总览页，可点任一项回去修改，确认后提交。刻意不提供「全部允许」之类的批量捷径——不看就同意等于没有审批。

部分拒绝时，已允许的调用照常执行，被拒的附拒绝说明回给模型，让它换个方式继续；全部拒绝则结束本轮。

### 动态规则积累

预先 allow 常用操作可减少审批频率，规则写法见 [权限配置指南](guides/permissions.md)。
...
MCP 服务器在**后台**连接，不阻塞会话打开：工具在连接完成后的下一轮对话自动可用。
失败的服务器会以 toast 轻提示，并可在 设置 → MCP 的服务器卡片状态徽标查看
（绿 = 已连接（悬停看工具数）/ 红 = 失败（悬停看原因）/ 灰呼吸 = 后台连接中）。
单个服务器的连接超时为 30 秒，超时/失败只跳过该服务器，不影响其它服务器与会话。
...
**配置阈值**（`summary_threshold: 0.7` 表示用量达模型上下文窗口 70% 时触发；窗口按当前模型自动取自 models.dev，与界面上的「上下文用量」环同一个分母）：
```json
{
  "token": {
    "summary_threshold": 0.7
...
**问题：** 程序崩溃或响应缓慢
**解决：**
1. 调低该模型的「单次输出上限」（设置 → 模型 → 编辑供应商 → 模型行的滑杆图标）；`agents.max_tokens` 只是探测不到时的兜底，改它对已被探测到的模型无效
2. 使用检查点持久化
3. 定期清理对话历史
...
1. **收紧单次工具结果的体量**（`context_length` 是工具结果大小上限的基准，调小即让单条/单轮工具输出更早被截断或卸载；压缩阈值不受它影响——那个分母是模型真实窗口）：
   ```json
   {
     "token": {
