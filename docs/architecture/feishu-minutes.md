...
**诊断判据是 `available` 而非 `tokenStatus`**：access_token 过期（2h）后状态转
`needs_refresh`，此时 token 仍可用——下次 user API 调用会自动刷新。认死
`tokenStatus == "valid"` 会让每次闲置超时后的诊断都谎报「授权已失效」，把用户支去重新
扫码。真未授权时 CLI 给 `available: false` 且**不带 `tokenStatus` 字段**（原判据靠
`.get()` 返回 `None` 才碰巧拦住，属巧合而非契约）。
...
两个会话。飞书没有 open_id → p2p chat_id 的查询 API（`im.v1` 只有建群/查群），唯一另
一条路是发一条消息从 `CreateMessageResponseBody.chat_id` 读回，代价是多一条可见消息。
与其为推送侧补一层反查（还需依赖"用户先私聊过"），不如让私聊直接以 open_id 为唯一 key。

投递地址仍走 `pool.chat_ids`（入站回填的真实 `oc_`），缺失时用 open_id 直投——
`send_message_sync` 按前缀自行选 `receive_id_type`，两者都能到同一私聊。
