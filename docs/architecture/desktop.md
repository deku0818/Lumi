...
- **前端结构**（`CronPage.tsx` + `App.tsx`）：
  ...
  - **未读是派生的，不是累积的**：唯一的本地事实源是已读集合 `readRuns`（run 的 `thread_id` → true，持久化 localStorage），侧栏角标 = 该任务 `run_threads`（`list_cron_jobs` 带回的近期可跳转 run，窗口同 `MAX_CRON_RUN_THREADS`）减去 `readRuns`，与 `RunsRail` 蓝点同源。因此桌面端离线期间执行的 run 重连后照样算未读——事件累积做不到这点。正在看某任务时新到的 run 直接记为已读，避免角标在用户眼皮底下跳出来。
