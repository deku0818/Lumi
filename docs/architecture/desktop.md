...
`<attached-file>` 是显示层注入块：`text_cleaning` 将其从可见正文剥离（不污染气泡），`server/ws._extract_files` 用同一标签的正则从历史消息还原文件胶囊。标签名 `ATTACHED_FILE_TAG` 定义在 `lumi/utils/constants.py` 作单一事实源，前端发送侧的同名字面量须与之一致。气泡内文件渲染成品牌金描边胶囊（`<attached-file>` 不显示路径文字，仅文件名 + hover tooltip）。
