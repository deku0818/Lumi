...
│  channel_rpc.py  get/save_channel + 体检 ← desktop 经 WS RPC 调 │
...
- 认领流程：先发「✅ 已完成」**锚点卡**（流式卡片必须回复某条消息才能创建；锚点发送失败不
  drain，通知留队下 tick 重试）→ drain 精确归属的通知 → `run_turn(synthetic=True)` 注入合成
...
IM 长会话（一群/一人一个永久 thread）不走 Stop 钩子的增量 dream（`auto_dream_stop_hook`
对渠道 thread 前缀直接放行，见 `utils/thread_id.is_channel_thread`），改由本循环到点统一编排，
两阶段：

1. **Dream（串行）**：遍历 `pool.chat_ids` 里有新消息的 thread，各持自己的运行锁跑长会话
   dream（`consolidate_session_dream`，只综合当前会话、`notify=False` 静默）。忙 thread 整批
   重试（3 轮 × 180s），仍忙则本轮整个跳过（dream+summary 都不做，留到明天）。
2. **Summary（并发限流）**：对刚 dream **成功**的 thread 强制压缩历史
   （`AgentBridge.compact_thread`，见 `summary.md` 离线压缩节），`Semaphore(summary_max_concurrency)`
   限流防 429；压缩后 `hub.on_channel_activity` 通知 desktop 刷新。
...
形式留在摘要载体里，后续 dream（输入含完整 messages）仍可见并沉淀，丢的只是摘要未保留的细节。

**互斥**：dream 间的互斥不在本循环——所有 dream 都经 `_run_dream_fork` 内的 per-project
...
## 接入体检（setup.py + checks.py + scopes.py）

机器人不工作有三个彼此独立的成因——权限没开、事件没订、版本没发——**三者的表现完全一样**：
长连接照常连上，一条消息都收不到，且开放平台不报任何错。故做成逐项诊断，与妙记链路同构。

- **一次查询判全部四项**：`GET /application/v6/applications/me/app_versions` 的单条记录里
  同时带 `scopes`（已申请权限）、`event_infos[].event_type`（已订阅事件的 code）、`status`
  （版本审核态，1 = 通过）。
- **体检自己也要授权**：该接口需 `SETUP_SCOPES`（`application:application.app_version:readonly`），
  未开通回 `99991672`。故第①项单独识别这个错误码——凭证此时明明是有效的，报成「凭证无效」
  会把用户支去重抄 App Secret。修复链接用 `ALL_SCOPES` 一次预填体检 + 机器人全部权限，
  避免「开完这一个回来发现又缺别的」。
...
- **三色 `tone` 由后端定**（`ok|warn|error`，非 `ok`+`warn` 两个布尔）：两布尔能拼出
  `ok=False,warn=True` 这种非法组合，且前端拿到后第一件事就是拼回三态。前端只留配色表。
- **SDK 调用一律走 `lark_call_classified`**（`lark_call.py`），它返回 `(resp, code, reason)`：
  `code` 供按码分支（99991672 缺权限 / `NETWORK_ERROR` 请求未送达），`reason` 是给用户看的
  失败原因。体检的四种失败各有各的下一步动作，绝不能都归成「凭证无效」。
- **scopes.py 是单一事实源**：诊断比对、修复链接、文档三者同源，改权限只改这一处。
...
`bot/v3/info` 报成功、体检报失败，用户不知该信哪个。
...
当前目录（兜底）。连接状态以文字标签呈现（未启用 / 已停止 / 连接中 / 已连接 / 连接失败），失败态
文字转红并显示后端给的具体原因。凭证下方是**接入体检**（`CheckPanel`，与妙记诊断共用），
打开弹窗且已有凭证时自动查一次；没有独立的「测试连接」按钮，体检第①项即是。
...
| `channels/feishu/checks.py` | 诊断结果结构 `Check` + `blocked_tail`（与妙记共用） |
| `channels/feishu/bridge_pool.py` | 每 thread 常驻 bridge + 运行锁 + thread→chat 映射 |
| `channels/feishu/daily_dream.py` | 每日定时整理：dream 串行 → summary 并发限流压缩 |
...
| `channels/feishu/minutes.py` | 妙记事件订阅自愈 + 链路诊断 + 取数提示（详见 [`feishu-minutes.md`](./feishu-minutes.md)） |
| `channels/{config,store,manager}.py` | 配置模型 / sidecar 存储 / 进程级管理器 |
| `channel_rpc.py` | get/save_channel、diagnose_minutes / diagnose_feishu_setup RPC |
