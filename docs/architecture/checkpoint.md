...
每个 thread 最多保留 20 个 checkpoint，超出时淘汰最旧记录。`meta.json` 采用原子写入，损坏时自动备份为 `.bak` 后重置。
...
| Bridge 集成 | `lumi/agents/bridge.py` | 协调文件快照与 LangGraph 会话回退（`list_checkpoints` / `rewind_to_checkpoint`，供前端调用） |
