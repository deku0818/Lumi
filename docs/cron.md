...
Lumi 内置定时任务系统，让 Agent 能够按照设定的时间规则主动执行任务并投递结果。基于 APScheduler 实现调度，支持相对时间、一次性、固定间隔和 cron 表达式四种调度方式。
...
TUI 模式下，正在执行的任务会在输入栏右侧显示动画指示器（如 `▰▱▱ 每日总结 执行中`），多个任务并发时显示数量。

### 错过任务补偿

Scheduler 启动时会检查每个启用的任务是否有在离线期间错过的执行（coalesce 策略）：
...
| 通道 | 说明 |
|------|------|
| TUIDelivery | 将结果持久化到 `NotificationStore`（JSON 文件），通过 `/cron-notify` 命令查看 |
...
定时任务按工作目录隔离存储。每个工作目录通过 `SHA256(resolved CWD)[:12]` 生成唯一标识，数据存储在 `~/.lumi/cron/{workspace_id}/` 下。不同项目目录的定时任务互不干扰。

目录下的 `workspace.meta` 文件记录了原始工作路径和创建时间，便于调试。

### 任务存储

任务持久化到 `~/.lumi/cron/{workspace_id}/jobs.json`，格式：
...
每个任务的执行记录存储在 `~/.lumi/cron/{workspace_id}/runs/{job_id}.jsonl`，JSONL 格式（每行一条），超过 2MB 自动裁剪旧记录。
