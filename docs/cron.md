...
2. 将任务的 `prompt` 作为输入，`tool_mode` 设为 `auto`（跳过人工审批）
...
任务执行结果通过 `DeliveryManager` 广播到所有已注册的投递通道。当前内置两种：

| 通道 | 说明 |
|------|------|
| TUIDelivery | 在终端界面显示 toast 通知（截取前 200 字符） |
| APIDelivery | 通过 SSE 推送，无活跃连接时缓存结果（最多 50 条） |
