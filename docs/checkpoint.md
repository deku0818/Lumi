...
6. **LangGraph 会话回退**：通过指向目标 checkpoint_id 实现分支，同时清理目标之后的所有 LangGraph checkpoint 数据（`aprune_checkpoints_after`）；回退到第一条消息之前时删除整个 thread
7. **残留状态恢复**：`AgentBridge` 在每次流式调用前检测残留的图状态——若 `state.next` 非空且无 interrupt（即上一轮被中断的 stale 状态），沿 parent 链回退到最近的 clean checkpoint，确保回滚时不包含中断轮次的消息
