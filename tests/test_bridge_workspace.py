...
async def test_set_workspace_does_not_change_process_cwd(tmp_path):
    ...
    old_cwd = Path.cwd()
    bridge = AgentBridge()
    result = await bridge.folders.set_workspace(str(tmp_path))
...
async def test_set_workspace_rejects_missing_dir(tmp_path):
    ...
    with pytest.raises(ValueError):
        await bridge.folders.set_workspace(str(tmp_path / "nope"))
...
def test_add_remove_folder(tmp_path):
    ...
    assert bridge.folders.add_folder(str(extra))["folders"] == [resolved]
    # 重复添加去重
    assert bridge.folders.add_folder(str(extra))["folders"] == [resolved]
    assert bridge.folders.remove_folder(str(extra))["folders"] == []


def test_add_folder_rejects_missing_dir(tmp_path):
    bridge = AgentBridge()
    with pytest.raises(ValueError):
        bridge.folders.add_folder(str(tmp_path / "nope"))
